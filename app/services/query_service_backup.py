...
from app.services.query_intent_analyzer import QueryIntentAnalyzer  # Add intent analyzer
import pandas as pd
import time
...
class QueryService:
    def __init__(self, db: Session):
        ...
        self.intent_analyzer = QueryIntentAnalyzer()  # Add intent analyzer
        logger.debug("QueryService initialized with context support and intent analysis")
    ...
    async def process_context_aware_query(
        ...
        try:
            # Step 0: Analyze query intent and scope
        ...
        if dataset_id:
            ...
            if not profile_name:
                # Use intent analysis for smart agent routing
                recommendations = intent_analysis['processing_recommendations']
                profile_name = recommendations.get('agent_profile', 'general_analyst')
                logger.info(f"Auto-selected agent profile: {profile_name} based on intent analysis")
                
                # Fallback to existing validation logic if needed
                if profile_name == 'general_analyst':
            ...
            if ResponseHandler.is_success(query_result):
                ...
                enhanced_data = {
                    **result_data,
                    ...
                    "intent_analysis": intent_analysis,  # Add intent analysis
                    "processing_recommendations": intent_analysis['processing_recommendations']
                }
